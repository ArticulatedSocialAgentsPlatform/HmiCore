...
public class TechniqueCommonLight extends ColladaElement {
   ...
   public StringBuilder appendContent(StringBuilder buf, XMLFormatting fmt) {
      ...
      appendXMLStructure(buf, fmt, pointLight);
      appendXMLStructure(buf, fmt, directional);
      appendXMLStructure(buf, fmt, spot);
      return buf;  
   }
   ...
   public void decodeContent(XMLTokenizer tokenizer) throws IOException {
      while (tokenizer.atSTag()) {
         ...
         } else if (tag.equals(Point.xmlTag()))  {                
                 pointLight = new Point(getCollada(), tokenizer);  
         } else if (tag.equals(Directional.xmlTag()))  {                
                 directional = new Directional(getCollada(), tokenizer);   
         } else if (tag.equals(Spot.xmlTag()))  {                
                 spot = new Spot(getCollada(), tokenizer);          
         } else {     
            super.decodeContent(tokenizer);    
            getCollada().warning(tokenizer.getErrorMessage("TechniqueCommon (Light): skip : " + tokenizer.getTagName()));
            tokenizer.skipTag();
         }
      }
      addColladaNode(ambient);   
      addColladaNode(pointLight); 
