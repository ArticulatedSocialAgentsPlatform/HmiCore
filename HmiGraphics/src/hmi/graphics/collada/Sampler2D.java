...
public class Sampler2D extends ColladaElement
{
    ...
    // child elements:
    private String source; // required, must be the sid of a surface
    private String wrapS; // default value: WRAP( = repeat) other value: MIRROR, CLAMP (=clamp_to_edge), BORDER (=clamp_to_border), NONE
                          // (=clamp_to_border/decal)
    private String wrapT; // default value: WRAP only for sample2D sampler3D, samplerCUBE, samplerDEPTH
    private String wrapP; // default value: WRAP only for sample3D, samplerCUBE, samplerRECT
    ...
    public Sampler2D(Collada collada, XMLTokenizer tokenizer) throws IOException
    {
        super(collada);
        readXML(tokenizer);
    }
    ...
    {
        appendTextElement(buf, "source", source, fmt);
    ...
    public void decodeContent(XMLTokenizer tokenizer) throws IOException
    {
        while (tokenizer.atSTag())
        {
            ...
            {
                source = tokenizer.takeTextElement("source");
            }
            ...
            {
                getCollada().warning(tokenizer.getErrorMessage("Sampler2D: skip : " + tokenizer.getTagName()));
                tokenizer.skipTag();
            }
        }
